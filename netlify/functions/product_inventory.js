...
const { createInventoryItem } = require('../../utils/inventory');
//#endregion
...
const getReceiptsWithCylinders = async (receiptsWithoutInventory) => {
  return await Promise.all(
    receiptsWithoutInventory.map(async function (receipt) {
      ...
      receipt.cilindros = cylindersData.results.map((item) =>
        mapCylinders(item, cylindersCameFrom.inventory)
      );
      return receipt;
    })
  );
